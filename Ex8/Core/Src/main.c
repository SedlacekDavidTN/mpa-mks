...
int pass_cmp(uint8_t *i) {
	uint8_t pass[5] = {7, 9, 3, 2, 12};
...
int main(void)
{
	...
	while (1)
	{
		...
		static uint32_t timeSample;
		static uint8_t lockTimeout = 1;

		tick = HAL_GetTick();

		if (tick > (timeSample + 1000) && (key != -1)) {
			timeSample = tick;
			lockTimeout = 0;
		...
		if (tick > timeSample + LOCK_TIMEOUT && lockTimeout == 0) {
			lockTimeout = 1;
			printf("Lock timeout\n");
			i = 0;
		}
